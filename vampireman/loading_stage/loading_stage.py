...
pipeline run. First, a `vampireman.data_structures.State` object is created with all its default values in place. Then,
if the user provided a settings file via the `--settings-file` command line parameter, the given yaml file is read and
another `vampireman.data_structures.State` object with the values from the yaml is instantiated. Afterwards, the first
object is overridden with the second one to keep the default settings without the user having to explicitly specify
everything.
"""
